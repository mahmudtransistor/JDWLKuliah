...
public class DatabaseSabtu extends SQLiteOpenHelper {

	//Versi database
			private static final int VERSI_DATABASE =1;
			//Deklarasi nama database
			private static final String DATABASE_NAME = "dbsabtu";
	public DatabaseSabtu(Context context) {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		//Perintah SQL untuk membuat Tabel Jadwal senin
				String CREATE_TABEL_SABTU = "CREATE TABLE tb_sabtu ( " +
	...
	public void addSabtu(Sabtu sabtu){
		//1.Dapatkan Referensi kedatabase
				SQLiteDatabase db = this.getWritableDatabase();
				//2.Buta object content Values untuk menambah kolom atau value
				ContentValues values = new ContentValues();
				values.put(KEY_NMMATKUL, sabtu.getNm_matkul());
				values.put(KEY_KELAS, sabtu.getKelas());
				values.put(KEY_RUANG, sabtu.getRuang());
				values.put(KEY_JAM, sabtu.getJam());
				//3. jalan kan perintah insert
				db.insert(TABEL_SABTU, null, values);
				//pesan LOG untuk menambah data Jadwal
				Log.i("Tambah data jadwal dari method adaJadwal", sabtu.toString());
				//4. Tutup database
				db.close();
	}
	//Getjadwal untuk senin
			public Sabtu getSabtu(int id){
				//1. Dapatkan Referensi kedatabase
				SQLiteDatabase db = this.getReadableDatabase();
				//2. Exsekusi Query database simpan hasil nya ke Object Kursor
				Cursor cursor = 
						db.query(TABEL_SABTU,
								COLUMNS, 
								"id= ?",
				...
				if(cursor !=null)
					...
					sabtu.setId(Integer.parseInt(cursor.getString(0)));
					sabtu.setNm_matkul(cursor.getString(1));
					sabtu.setKelas(cursor.getString(2));
					sabtu.setRuang(cursor.getString(3));
					sabtu.setJam(cursor.getString(4));
					//pesan LOG mengganil data Jadwal (1 Record)
					Log.i("ambil data jadwal ("+id+") dari method getJadwal : ", sabtu.toString());
				
					return sabtu;
				
			}
			//ambil semua Jadwal senin
			public ArrayList<Sabtu> getAllSabtu(){
				//membuat object list jadwal
				ArrayList<Sabtu> tb_sabtu = new ArrayList<Sabtu>();
				//1. Buat Query buku
				String query = "SELECT * FROM " + TABEL_SABTU;
				...
				Log.i("Ambil semua buku dari method getAllBooks", tb_sabtu.toString());
				db.close();
				return tb_sabtu;
			...
			public List<String> getAllSelasaTittle(){
				List<String> tb_sabtu = new LinkedList<String>();
				...
				if(cursor.moveToFirst()){
					do{
						tb_sabtu.add(cursor.getString(1));
			...
			public int UpdateSabtu(Sabtu sabtu){
				//1. Dapatkan referensi kedatabase
				SQLiteDatabase db = this.getWritableDatabase();
				//2.Buta object content Values untuk menambah kolom atau value
				ContentValues values = new ContentValues();
				...
				int i = db.update(TABEL_SABTU, values,
					      KEY_ID+"=?",
					      new String[]{String.valueOf(sabtu.getId())});
				//Pesan LOG untuk mengupdata data buku
				Log.i("Updata data jadwal",sabtu.toString());
				db.close();
				return i;
			...
			public void DeleteSabtu (Sabtu sabtu){
				SQLiteDatabase db = this.getWritableDatabase();
				//perintah untuk manghapus
				db.delete(TABEL_SABTU, KEY_ID + "=?",
						new String[]{String.valueOf(sabtu.getId())});
				db.close();
				//pesan LOG untuk manghapus data dari method deletejadwal
				Log.i("Hapus data dari method DeletelJadwal()", sabtu.toString());
			}
